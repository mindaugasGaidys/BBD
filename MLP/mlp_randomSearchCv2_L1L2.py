...
def create_model(hidden_layers=1, hidden_units=16, dropout_rate=0.5, learning_rate=0.001, patience=5, batch_size=64):
    ...
    model.add(Flatten())
    model.add(Dense(hidden_units, input_shape=(max_length,), kernel_regularizer=l1_l2(l1=0.01, l2=0.01)))
    model.add(LeakyReLU())
    model.add(Dropout(dropout_rate))
    
    for _ in range(hidden_layers - 1):
        model.add(Dense(hidden_units, kernel_regularizer=l1_l2(l1=0.01, l2=0.01)))
        model.add(LeakyReLU())
        model.add(Dropout(dropout_rate))
